...
# Make `from foundation import filesys` resolve whether this module is imported as
# `foundation.models` or run as a script (`python3 foundation/models.py --route ...`),
# where sys.path[0] is foundation/ rather than the repo root.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from foundation import filesys, proc  # noqa: E402


# ---------------------------------------------------------------------------
# Model alias tables — DATA, loaded from app/config/models.yml (state out of
# code), via the foundation.filesys facade. The literal tables moved to YAML; the
...
def model_id_for_route(route: str) -> str:
    ...
    source of truth for every gen-* alias resolution.
...
def _backend_anthropic(
    ...
    # Configured model ids may carry a litellm provider prefix; strip it for the API.
...
def _backend_openrouter(
    ...
    """OpenRouter (OpenAI-compatible chat completions). Lets callers reach a
    genuinely non-Anthropic model family (gpt-*, gemini-*, llama-*, …) for
    cross-model comparisons. The model id is the bare OpenRouter id; accepts an
    explicit ``or:<id>`` alias, else falls back to OPENROUTER_MODEL."""
