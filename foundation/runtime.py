"""runtime.py — generic runtime / OS plumbing for foundation (stdlib-only leaf).

Reusable mechanisms with **no** pipeline/domain knowledge. Each is a named
pattern a consumer can adopt without dragging caller policy along:
...
imports from ``foundation.*``, so anything may import it without cycle risk.
"""
