"""structures.py — generic data structures for foundation (stdlib-only leaf).
...
YAML config or issue bodies). That keeps the *mechanism* in foundation and the
...
# ===========================================================================
# Issue dependency DAG — a thin DOMAIN layer over Graph (was foundation/dag.py).
#
# Parses GitHub issue cross-references to build a dependency graph; callers
# use it to order work items from foundational to dependent, or to render the
# dependency structure as JSON / Mermaid / Markdown. Construction is
# deterministic: identical (items, patterns) yield an identical graph. Issue
# text is parsed as DATA — only `#<number>` cross-references are read.
#
...
class IssueGraph:
    ...
    # have no in-window node to point at, so they are absent from `deps`; callers
    # must treat any issue with a non-empty entry here as BLOCKED rather than
    # silently dropping the unresolved dependency.
...
def build(items: List[Dict[str, Any]], *, fwd, rev) -> IssueGraph:
    """Build the issue dependency graph from a list of issue items.
    ...
    # leave the dependent looking foundational and let a caller process it ahead
    # of its real blocker. Record those unresolved blockers separately so callers
    # can treat the dependent as BLOCKED (conservative).
...
def to_json(graph: IssueGraph, *, source: str = "") -> Dict[str, Any]:
    """Canonical machine view: nodes + edges + roots. Callers and external tooling
    read this."""
    return {
        "source": source,
