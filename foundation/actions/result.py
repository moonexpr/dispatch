...
The variant is modelled on ``foundation.proc.Completed`` (``.ok`` + ``.check()``) so it
reads natively next to the rest of foundation. ``Output`` carries a ``value`` plus
...
class ActionError(RuntimeError):
    """Base for Action-layer failures. Mirrors foundation.proc.ProcError: a
