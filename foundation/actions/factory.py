...
What actually differs between the two:
  ...
  * inference runner — a deterministic oracle/fixture (no model, no network) vs.
                       ``foundation.models.chat`` (a real query()).
...
Leaf module: stdlib + sibling foundation leaves (foundation.models is imported lazily,
...
# Default route -> alias map for the real backend. Callers may pass explicit
# aliases instead of gen-* routes to override these defaults.
...
class AbstractActionFactory(ABC):
    ...
    def deserialize(self, script: Any, *, name: str = "engineering") -> Action:
        ...
        phases chain; ``parallel: true`` fans out under one parent; each phase
        is wrapped in a permission Governor (the script's allowlist) and a budget
        Governor (the phase's share of the script's total budget)."""
    ...
    def _build_script_body(self, script: OrchestrationScript) -> Sequence:
        ...
        # All phases share one budget sub-meter (the script carries the total;
        # falls back to the sum of per-phase shares); each phase is also
        # pre-flight-checked against its own share via ``estimate``.
...
class MockActionFactory(AbstractActionFactory):
    ...
    charge/observe but never abort or deny. Use this for end-to-end tests of any
    Controller with zero real effects."""
...
class RealActionFactory(AbstractActionFactory):
    """Durable, enforcing family. Inference calls ``foundation.models.chat`` (a real
    query()); governors enforce budgets and deny-by-default permissions; shelves
    persist through ``foundation.filesys``. Under ``ctx.dry_run`` the inference runner
    emits a deterministic placeholder instead of calling a model."""
    ...
    def _inference_runner(self, spec: InferenceSpec, payload: Any, ctx: Any) -> Output:
        if getattr(ctx, "dry_run", True):
            ...
            # deterministic oracle, honor it so dry-run output matches the structured
            # shape downstream consumers expect. Specs with no oracle fall back to a
            # textual placeholder.
            if spec.oracle is not None:
                value = spec.oracle(payload, ctx)
