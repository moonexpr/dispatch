...
backend, routing every key through ``foundation.filesys`` so it honours the
``DISPATCH_APP_DIR`` override and the absolute-path escape hatch — it never calls
bare ``open()``.

Leaf module: imports only stdlib + ``foundation.filesys`` (itself a leaf).
"""
...
class FileShelf(Shelf):
    ...
    ``foundation.filesys`` so the app-dir override and absolute-path escape hatch
