...
Three kinds; each enforces a cross-cutting policy by decoration:

  * ``BudgetGovernor``     — a usage-reading gate (token budget / cost cap).
  * ``PermissionGovernor`` — deny-by-default tool/operation gate.
  * ``IterationGovernor``  — bounds repeated invocation (cumulative cap).
...
class BudgetGovernor(Governor):
    """Charges a scoped ``BudgetMeter`` (parented to the global meter on ``ctx``)
    ...
    scoped cap and the global total are both enforced by one charge. ``estimate``
    is the pre-flight cost guess; actual cost is taken from ``result.meta['usage']``
    if the runner reported it."""
