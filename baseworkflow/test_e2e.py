...
matching the shell-harness convention. Run: ``python3 baseworkflow/test_e2e.py``.
"""
...
def test_run_live_dry_run_matches_mock() -> None:
    ...
    deterministic dry-run path. We prove (c) by detonating ``foundation.models.chat``
    for the duration of the run: any real inference would raise, failing the run."""
    import foundation.models as _models

    sentinel = {"called": False}

    def _boom(*a, **kw):  # any real model call trips this
        sentinel["called"] = True
        raise AssertionError("run_live(dry_run=True) called foundation.models.chat (real model)")
...
def test_yaml_workflow() -> None:
    ...
    import bindings
    from foundation.actions import MockActionFactory, Statechart
    from foundation.workflow import (
        ActionRefNode,
        LoopNode,
        ParallelNode,
        PhaseNode,
    ...
    from foundation.workflow.manifest import ActionManifest, IORef
