...
(no pytest). Run: ``python3 baseworkflow/test_slice_dispatch.py``.
...
_HERE = os.path.dirname(os.path.abspath(__file__))   # baseworkflow
_ROOT = os.path.dirname(_HERE)                        # repo root
for _p in (_ROOT, _HERE, os.path.join(_HERE, "subsystems"), os.path.join(_HERE, "bindings")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

import baseworkflow  # noqa: E402  (sets up package paths + registers bindings)
from foundation.actions import AgentSpec, MockActionFactory, OrchestrationScript, PhaseSpec, Program  # noqa: E402
...
def test_dryrun_program_still_runs_green() -> None:
    ...
    from foundation.actions import BudgetMeter, Context, MemoryShelf, Shelves
