...
Two related-but-distinct workflows bind their ``engineer:`` tokens here:
  ...
  * **the engineer unit-of-work** (``app/workflows/engineer.yml``) — the SDK Engineer
    that turns one Job Request into one Invoice, compiled by ``foundation.workflow``
...
-------------------------------------
  ...
  * the model run is the one **inference** (``engineer:run``) — the agent leaf whose
    *runner* is injected by the factory (``EngineerFactory``), the inversion-of-
    control hinge ``foundation/actions/action.py`` describes. Swap the factory and the
...
from foundation import models, proc
...
from foundation.workflow import TokenRegistry, compile_workflow, load_workflow
...
# of truth. ``seam`` (baseworkflow/subsystems/seam.py) resolves via the bindings-package /
...
def _model_for_route(route: str) -> str:
    ...
    truth (foundation.models). Falls back to the sonnet tier id so the engineer
...
def _agent_definitions(units: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    prompt, tools}}`` shape ``foundation.agent_sdk`` turns into ``AgentDefinition``
...
# EngineerAgent.SYSTEM_PROMPT (agents/EngineerAgent.py, founded on engineer.md /
...
# generic, agent-agnostic ``foundation.agent_sdk`` — reached via its make_runner()
...
def _engineer_worker(spec: InferenceSpec, payload: Any, ctx: Context) -> Output:
    ...
    Invoice."""
    from agents import EngineerAgent  # lazy: the repo root is on sys.path by bind time
...
def build_registry() -> TokenRegistry:
    ...
    --registry baseworkflow.bindings.engineer:build_registry``."""
    reg = TokenRegistry()
    register_unit(reg)
    return reg
...
def _orchestration_phases(script: Any) -> List[Any]:
    ...
    if not script:
        return []
    try:
        from foundation.actions import OrchestrationScript
