...
``foundation.filesys`` facade. This module is the load seam ONLY — it holds no
...
_ROOT = os.path.dirname(os.path.dirname(_HERE))  # repo root (architect -> ..)
# Make `from foundation import filesys` importable when load() runs, regardless of
# how this module was reached. Path-only (no import side effects) so the
# degrade-to-skeleton contract below still holds if foundation/PyYAML are absent.
if _ROOT not in sys.path:
    sys.path.insert(0, _ROOT)

# Default config location, relative to the app dir. An absolute
# DISPATCH_WORKPLAN_RULES override is honoured as-is by foundation.filesys.resolve.
...
def load() -> Dict[str, Any]:
    ...
    try:
        from foundation import filesys  # lazy: missing engine/PyYAML -> skeleton
