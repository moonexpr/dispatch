...
``foundation.runtime`` (and subprocess invocation to ``foundation.proc``) so they can be
...
``foundation.structures`` and exposed by ``statemachine.py``; this module no longer
...
from foundation import proc, runtime


# ---------------------------------------------------------------------------
# Repo-root resolution. This file lives at <root>/baseworkflow/subsystems/,
# so repo root is parents[2]: subsystems -> baseworkflow -> <root>.
# ---------------------------------------------------------------------------
...
# foundation.runtime; the policy — *which* four vars are protected — stays here.
# ---------------------------------------------------------------------------
runtime.load_dotenv(
...
# websitewf) instead. The historical ``visitor`` authoring fallback is
...
LEDGER_PY = os.environ.get("LEDGER_PY", str(PIPELINE_ROOT / "visitor" / "ledger" / "ledger.py"))
PREP_PY = os.environ.get("PREP_PY", str(PIPELINE_ROOT / "visitor" / "architect" / "prep.py"))
...
# lives in foundation.runtime.Logger; the subsystem-resolution policy stays here.
def _subsys() -> str:
...
# timed acquire, graceful degradation) lives in foundation.runtime.file_lock; the
