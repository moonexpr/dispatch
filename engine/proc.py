"""proc.py — subprocess invocation & resource management for the pipeline.

Centralises *how* the pipeline shells out to a child process so client modules
(the classifier, gh reads, the engineer/prep launchers, the agent CLI backend, …)
describe *what* to run, not the capture / decode / timeout / cleanup plumbing.
Every project module that spawns a process goes through here rather than touching
...
class Completed(subprocess.CompletedProcess):
    """A finished process: ``subprocess.CompletedProcess`` + engine-native helpers.

    Subclassing keeps every attribute callers already use (``args``,
    ``returncode``, ``stdout``, ``stderr``) while adding the ``.ok`` / ``.check()``
    shape the rest of the engine reads (mirrored by ``engine.actions.result``).
    """
...
class Proc:
    ...
    Wraps a live :class:`subprocess.Popen` with the engine's vocabulary so a
...
def popen(cmd, *, env=None, cwd=None, text=True,
    ...
    here is never leaked::
        ...
        with proc.popen([engineer, *args], cwd=clone) as p:
