#!/usr/bin/env python3
"""engine/env.py — a broad, consistent environment for dispatch.

Dispatch's configuration (tokens, repo, tuning) can live in several places: the
...
resolves the same way no matter where a value sits, and so an agent can *search*
...
from engine import proc  # capturing subprocess wrapper (ProcError on launch failure)
...
class DotEnvSource(EnvSource):
    ...
    ``engine.runtime.load_dotenv`` (``export`` prefixes, ``#`` comments and quotes
...
class KeychainSource(EnvSource):
    ...
    any key stored as a generic password whose *service* is the key name; dispatch
    ...
    def get(self, key: str) -> Optional[str]:
        # Try the dispatch-namespaced entry first, then a service-only entry so a
    ...
    def items(self) -> Dict[str, str]:
        # Only the keys dispatch itself saved (via the index) — never a full dump.
        out: Dict[str, str] = {}
...
class Environment:
    ...
    def default(cls, *, repo_root: Optional[Union[str, Path]] = None,
        ...
        """The standard dispatch stack: process env (highest priority) → ``.env`` →
...
#   python -m engine.env get VERCEL_ACCESS_TOKEN          (prints the value)     #
#   python -m engine.env query vercel                     (masked matches)       #
#   python -m engine.env find vercel supabase             (token by hint)        #
#   python -m engine.env tokens                            (all token-shaped)     #
#   python -m engine.env save --store keychain [KEY ...]   (persist secrets)      #
# Note: `get` prints the raw value (for piping); the others mask by design.       #
# --------------------------------------------------------------------------- #
def main(argv: Optional[List[str]] = None) -> int:
    import argparse

    ap = argparse.ArgumentParser(description="Search and persist dispatch's broad environment.")
