#!/usr/bin/env python3
"""engine.agents.base_agent — the abstract agent archetype (abstraction side of a Bridge).
...
:func:`engine.agent_sdk.make_backend`. Swap the backend without touching the definition.

Concrete archetypes (``EngineerAgent`` / ``ArchitectAgent`` / ``AdminAgent``) live in
the app layer at ``src/agents/``; this abstract base + the backend backends live in
``engine`` so the foundation owns the machinery. Leaf-module discipline: stdlib +
sibling ``engine`` modules only; never import from ``src``.
"""
...
from engine import agent_sdk, env as _env, models, runtime as _runtime
...
class BaseAgent(ABC):
    ...
    #: ``engine.models.model_id_for_route``. A call may pass an explicit ``model`` or a
    ...
    def resolve_model(self, *, model: Optional[str] = None, route: Optional[str] = None) -> str:
        ...
        (``engine.models``). Never returns empty — falls back to the sonnet tier."""
        if model:
            return model
        if self.model:
            return self.model
