...
"""__main__.py — the operator-facing "test your YAML" CLI.
    ...
    python -m engine.workflow <workflow.yml> [--registry module:function]
...
hard-wire any ``src`` import — ``engine`` stays a leaf.
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(prog="engine.workflow", description="Validate a workflow YAML (format test).")
