#!/usr/bin/env python3
"""engine.worker — Workers: executors that SEAT a unit of work (an agent or a proc).

A **Worker** is the executor that decouples *running* from *what is run*. A concrete
worker seats one kind of work:

  * :class:`AgentWorker` seats a :class:`engine.agents.BaseAgent` — an LLM agent
...
:class:`engine.agents.AgentOutcome`, the proc into a :class:`ProcOutcome`. Binding code
hands a worker the work it is binding and the domain text, and the worker owns the run
lifecycle, the shelf record (lifetime updates), the audit :class:`engine.actions.Output`
(event propagation), and the timeout. Concrete workers are created through the
:func:`make_worker` factory.
"""
...
class ProcOutcome:
    ...
    :class:`engine.agents.AgentOutcome`. ``is_error`` covers a non-zero exit, a timeout
    (``timed_out=True``), or a spawn failure; ``error`` carries the message."""
...
class AgentWorker(BaseWorker):
    """Seats a :class:`engine.agents.BaseAgent`. :meth:`run` / :meth:`invoke` return the
    raw :class:`AgentOutcome` (for fail-safe producers that own their deliverable I/O);
    :meth:`execute` runs the full inference pattern — write the run record onto the shelf
    and return the audit :class:`Output` — so the binding writes no shelf state itself."""
...
class ProcWorker(BaseWorker):
    ...
    def run(
        ...
        *,
        ...
        """Run one argv through :func:`engine.proc.run` (the process-management library)
        with this worker's cwd/env/timeout, folding a timeout, launch failure, or
        non-zero exit into a never-raising :class:`ProcOutcome`."""
...
# ``engine.agent_sdk._BACKENDS`` (backends) and ``agents._ARCHETYPES`` (agents).
