...
The ``Context`` threads execution state through a run the way ``TickContext``
threads a stage walk in ``src/orchestration/visitors.py`` — one mutable record
shared across the whole tree (shelves, budget meter, dry-run flag, audit trace,
iteration counters).

Leaf module: imports only stdlib + sibling engine leaves.
"""
...
from engine.runtime import Logger
...
class BudgetMeter:
    ...
    charges its parent, so a per-phase meter (ENGINEERING_BUDGET) and the global
    workflow meter (1,000,000) are both enforced by the same call. Pure
    mechanism — domain budget figures live in the workflow, not here."""
...
class Context:
    ...
    shelves: Shelves
    meter: BudgetMeter
    log: Logger = field(default_factory=lambda: Logger("engine.actions"))
    dry_run: bool = True
    depth: int = 0
    permissions: Any = None
    trace: List[Dict[str, Any]] = field(default_factory=list)
    counters: Dict[str, int] = field(default_factory=dict)
    # Per-phase sub-meters (architect/engineering/admin), each a child of ``meter``
...
class Procedure(Action):
    ...
    The catalog wraps the repo's pure subsystem functions (approval.approve,
    decompose.plan, …) as Procedures — they ARE deterministic programs."""
...
class Inference(Action):
    ...
    factory's runner calls ``engine.models.chat``. The Action is identical either
