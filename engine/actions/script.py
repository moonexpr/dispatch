...
The Architect's A6 step emits an *orchestration script*: a declarative, JSON-able
description of an agent team and how its phases wire together. The Administrator's
M4 step hands that script to ``factory.deserialize(...)``, which rebuilds it into
an executable ``Program``. The script itself is pure data — no callables, no
model handles — so it survives a round-trip through any Shelf (including the
JSON-backed ``FileShelf``). The *runner* that actually fulfils each phase (mock
oracle vs. real model) is injected by the factory at deserialize time, never
baked into the script. That is what lets one script run mock or real unchanged.

``InferenceSpec`` is the runtime spec an ``Inference`` action carries. It is built
either directly (the Architect/Admin sub-actions) or from a script ``AgentSpec``
at deserialize time. It can carry an ``oracle`` (a deterministic stand-in the mock
runner calls) or a ``fixture`` (a canned output) — these are the seams that let
the end-to-end mock test exercise real logic with no model and no side effects.
...
class OrchestrationScript:
    ...
    budget: int = 0  # the engineering bucket (ENGINEERING_BUDGET); 0 -> sum of phases
