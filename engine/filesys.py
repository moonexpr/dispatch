#!/usr/bin/env python3
"""filesys.py — the pipeline's file-access facade, rooted at the app directory.

Every durable, declarative resource the pipeline reads (the config YAMLs in
``app/config/``, agent persona files, and any packaged data added later) is
reached through this one module instead of each caller re-deriving its own
...
lets the per-file override env vars (``DISPATCH_SELECTION_WEIGHTS``,
``DISPATCH_WORKPLAN_RULES``) keep handing in paths outside the app tree exactly
as they did before this facade existed.

Layering
--------
``engine`` is shared infrastructure; pointing it at ``app/`` is a deliberate,
single seam (``engine/models.py`` already reached into the app's config — this
module is where that coupling now lives, named and overridable, rather than
scattered). It is a **leaf** module: stdlib + an in-function PyYAML import only,
with no imports from other ``engine/*`` or ``src/*`` modules, so anything may
import it without cycle risk and without forcing PyYAML on text/bytes callers.
"""
...
# repo root = parent of engine/ ; the app dir is <repo>/app unless overridden.
