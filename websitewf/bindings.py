...
WebsiteWF is its own engine, a sibling of ``baseworkflow`` that *inherits
everything baseworkflow builds*: :func:`build_registry` starts from baseworkflow's
fully-populated :class:`~foundation.workflow.TokenRegistry` and registers the web-only
...
Validate the overlay against this registry:
  ``python3 -m foundation.workflow app/workflows/websitewf.yml \\
    --registry websitewf.bindings:build_registry``
"""
...
# Repo root on sys.path so sibling packages and ``foundation.*`` resolve regardless of
# how this module is imported (mirrors baseworkflow's convention).
...
def _feature_routes(body: str):
    ...
    try:
        _SUB = os.path.join(_ROOT, "baseworkflow", "subsystems")
