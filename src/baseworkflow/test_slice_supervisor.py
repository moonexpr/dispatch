...
Run: ``python3 src/baseworkflow/test_slice_supervisor.py`` (exit 0 = pass), matching
the repo's pytest-free, self-asserting convention (cf. engine/actions/test_statechart.py).
"""
...
_ROOT = os.path.dirname(os.path.dirname(_HERE))  # <root>/src/baseworkflow -> <root>
for p in (_ROOT, os.path.join(_ROOT, "src")):
    if p not in sys.path:
        sys.path.insert(0, p)

from engine.actions.action import BudgetMeter, Context  # noqa: E402
from engine.actions.shelf import MemoryShelf, Shelves  # noqa: E402
