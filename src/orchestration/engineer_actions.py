#!/usr/bin/env python3
"""engineer_actions.py — the implemented work behind ``app/config/engineer.yml``.

The SDK Engineer's unit-of-work — one Job Request -> one Invoice — is **expressed
in YAML** (``app/config/engineer.yml`` + ``app/config/actions/engineer/*.yml``) and
compiled by ``engine.workflow`` into a statechart. This module is the *bind layer*
that YAML refers to: the action bodies + the terminal predicate, registered into a
``TokenRegistry`` by :func:`build_registry`. ``engineer_sdk.py`` is the thin
``ENGINEER_BIN`` shell that reads the request, calls :func:`run_live`, and emits the
Invoice on stdout.
...
-------------------------------------
  * the deterministic git/gh/scan steps are **procedures** (clone, branch, judge,
    contamination gate, push, open-PR, finalize);
  ...
  * the four Invoice outcomes are modelled by the declarative ``terminal_when:``
    early-exit (engine primitive; predicate ``engineer_terminal`` registered here):
      ...
      - ``partial`` — branch pushed but PR creation failed;
      - ``completed`` — the happy path the ``engineer:finalize`` step builds.
...
# -- A7: open ONE PR --------------------------------------------------------
def act_open_pr(_inputs: Any, ctx: Context) -> Any:
    """Body restates acceptance + ``Closes #<n>`` for auto-close. Branch pushed but
    PR creation failed -> terminal ``partial`` (work done, no PR to arm-merge)."""
    s = _setup(ctx)
    branch, repo, issue = s["branch"], s["repo"], s["issue"]
    units, route, model = s["units"], s["route"], s["model"]
    default_branch = _sh(ctx).get("default_branch")
    summary = _sh(ctx).get("summary") or ""
    ...
    pr = subprocess.run(
        [s["gh_bin"], "pr", "create", "--repo", repo, "--base", default_branch,
         "--head", branch, "--title", f"Implement #{issue}: {s['title']}",
         "--body", pr_body],
        capture_output=True, text=True,
    )
    ...
    _sh(ctx).update({"pr_number": pr_number, "pr_url": pr_url})
    return Output(None)


# -- A8: finalize the happy-path Invoice ------------------------------------
def act_finalize(_inputs: Any, ctx: Context) -> Any:
    """The last step: build the ``completed`` (or ``needs-human`` on residual
    contamination) Invoice. Returns the Invoice as the controller's Result value."""
    s = _setup(ctx)
    contamination_note = _sh(ctx).get("contamination_note") or ""
    summary = _sh(ctx).get("summary") or ""
    pr_number = _sh(ctx).get("pr_number")
    pr_url = _sh(ctx).get("pr_url") or ""
    branch = s["branch"]
    changed_lines = _sh(ctx).get("changed_lines") or 0
    final_status = "needs-human" if contamination_note else "completed"
    _log(f"#{s['issue']} -> {final_status} (PR #{pr_number}, {changed_lines} lines, {int(_elapsed(ctx))}s)")
    invoice = build_invoice(
        issue=s["issue"], repo=s["repo"], status=final_status, branch=branch, pr_number=pr_number,
        scope_actual=_sh(ctx).get("scope_actual") or s["scope"], route_used=s["route"],
        tokens_in=_sh(ctx).get("tokens_in") or 0, tokens_out=_sh(ctx).get("tokens_out") or 0,
...
def register(reg: TokenRegistry) -> None:
    """Register the engineer unit's action bodies + terminal predicate (mirrors the
    ``register(reg)`` convention of ``src/baseworkflow/bindings/*``)."""
...
def build_registry() -> TokenRegistry:
    ...
    --registry src.orchestration.engineer_actions:build_registry``."""
    reg = TokenRegistry()
    register(reg)
    return reg
