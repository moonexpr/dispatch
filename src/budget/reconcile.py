...
Reads dispatch's append-only JSONL run-ledger (E4-1, src/ledger/ledger.py)
...
CLI
---
  python3 src/budget/reconcile.py --ledger run-ledger.jsonl
  python3 src/budget/reconcile.py --ledger run-ledger.jsonl --by day
  python3 src/budget/reconcile.py --ledger run-ledger.jsonl --by week
...
sys.path.insert(0, os.path.dirname(_HERE))  # src/ — for tuning / architect
